    ...
    <div className="flex flex-col bg-background">
          ...
          <div className="relative z-10 mx-auto mt-14 grid max-w-7xl gap-6 px-6 md:grid-cols-2">
            {homePT.brandPillars.items.map((pillar, i) => (
              <SectionCard key={pillar.title} delay={i * 0.08} className={`rounded-[2rem] border bg-white/80 p-0 shadow-[0_20px_50px_rgba(10,17,40,0.12)] ${pillarBorder[i % pillarBorder.length]}`}>
                <div className="relative h-56 w-full overflow-hidden rounded-t-[2rem] bg-[#DCEEFF]">{pillar.image && <img src={pillar.image} alt={pillar.title} className="h-full w-full object-cover opacity-70" />}<div className="absolute inset-0 bg-gradient-to-t from-white via-white/25 to-transparent" /></div>
                <div className="p-7 md:p-8"><h3 className="font-heading text-2xl font-black tracking-tight text-[#0A1024]">{pillar.title}</h3><p className="mt-3 text-[#0A1024]/76">{pillar.description}</p></div>
              </SectionCard>
            ))}
          </div>
        </Section>
      ...
      <Section className={lightSectionClass}>
        ...
        <div className="relative z-10 mx-auto mt-14 grid max-w-7xl gap-7 px-6 lg:grid-cols-3 md:gap-9">
          {content.marketingAI.cards.map((card, i) => { const Icon = marketingIcons[i % marketingIcons.length]; return (
            <SectionCard key={card.title} delay={i * 0.1} variant="highlight" className="rounded-[2rem] border border-[#0A8AFF]/25 bg-white/82 p-8 text-[#0A1024] shadow-[0_20px_50px_rgba(10,17,40,0.12)] md:p-10">
              <div className="mb-7 inline-flex h-16 w-16 items-center justify-center rounded-2xl border border-[var(--brand-primary)]/40 bg-[var(--brand-primary)]/10 text-[var(--brand-primary)]"><Icon className="h-8 w-8" /></div>
              <h3 className="mb-4 font-heading text-3xl font-black tracking-tight text-[#0A1024]">{card.title}</h3>
              <ul className="space-y-3">{card.points.map((point) => <li key={point} className="flex items-start gap-3 text-[#0A1024]/82"><Icons.CheckCircle className="mt-0.5 h-5 w-5 shrink-0 text-[var(--brand-primary)]" /><span>{point}</span></li>)}</ul>
            </SectionCard>
          ); })}
        </div>
      </Section>
