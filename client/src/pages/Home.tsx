  ...
  return (
    <div className="flex flex-col bg-background">
      <Section className="relative flex min-h-[90vh] items-center justify-center overflow-hidden bg-modern-gradient tech-grid scanlines pt-28 pb-20 md:pt-40 md:pb-28">
        ...
        <TechParticleField className="opacity-80" />

        <motion.div
          aria-hidden="true"
          className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_50%_30%,rgba(0,212,255,0.3),transparent_45%)]"
        ...
        <div className="relative z-10 mx-auto max-w-6xl px-6 text-center">
          <Reveal>
            <div className="glass-panel cyber-border inline-flex items-center gap-3 rounded-full px-6 py-2.5 text-xs font-black uppercase tracking-[0.35em] text-[#00D4FF] sm:text-sm">
          ...
          <Reveal delay={100}>
            <h1 className="mt-10 text-4xl font-black leading-[0.9] tracking-tight text-white sm:text-6xl md:text-7xl lg:text-8xl drop-shadow-[0_0_35px_rgba(0,212,255,0.35)]">
              {content.hero.title}
            </h1>
          </Reveal>

          <Reveal delay={180}>
            <p className="mx-auto mt-8 max-w-4xl text-lg leading-relaxed text-white/85 sm:text-xl md:text-2xl">
      ...
      <Section className="relative overflow-hidden bg-blue-tint tech-grid py-24 md:py-36">
        <TechBackdrop intensity="soft" className="opacity-70" />
        <div className="pointer-events-none absolute inset-0 dots-matrix opacity-25" />

        <SectionHeader>
          <Reveal>
            <SectionTitle label={content.values.label} title={content.values.title} className="text-foreground" />
          </Reveal>
        </SectionHeader>
          ...
          {content.values.items.map((value, i) => {
            ...
            return (
              <SectionCard key={value.title} delay={i * 0.08} className="glass-panel cyber-border rounded-3xl p-7 md:p-8">
                ...
                <p className="text-base leading-relaxed text-foreground/75">{value.description}</p>
              </SectionCard>
            );
          })}
        </div>
      ...
      <Section id="services" className="relative overflow-hidden bg-modern-gradient tech-grid scanlines py-24 md:py-36">
        <TechBackdrop intensity="medium" />
        <TechParticleField className="opacity-55" />

        <SectionHeader>
          <Reveal>
            <SectionTitle label={content.services.label} title={content.services.title} className="text-white" />
          </Reveal>
        </SectionHeader>
        ...
        <div className="relative z-10 mx-auto mt-16 grid max-w-7xl gap-7 px-6 lg:grid-cols-3 md:gap-9">
          {content.services.items.map((service, i) => {
            ...
            return (
              <SectionCard key={service.title} delay={i * 0.1} variant="highlight" className="glass-panel cyber-border rounded-[2rem] p-8 md:p-10 bg-[#06102A]/55">
                <div className="mb-8 inline-flex h-16 w-16 items-center justify-center rounded-2xl border border-[#3B82F6]/55 bg-[#3B82F6]/15 text-[#22D3EE] shadow-[0_0_26px_rgba(34,211,238,0.32)] transition-all duration-500 group-hover:scale-110 group-hover:bg-[#00D4FF]/20">
                  <Icon className="h-8 w-8" />
                </div>

                <h3 className="mb-4 text-3xl font-black tracking-tight text-white">{service.title}</h3>
                <p className="text-lg leading-relaxed text-white/80">{service.description}</p>
              </SectionCard>
            );
          })}
        </div>
      ...
      <Section className="relative overflow-hidden bg-ice py-24 text-center md:py-40 tech-grid">
        <TechBackdrop intensity="soft" />

        <div className="relative mx-auto max-w-5xl px-6">
          <Reveal>
            <h2 className="text-4xl font-black tracking-tight text-foreground sm:text-6xl md:text-7xl">{content.cta.title}</h2>
          </Reveal>

          <Reveal delay={100}>
            <p className="mx-auto mt-8 max-w-4xl text-lg leading-relaxed text-foreground/75 sm:text-2xl">{content.cta.description}</p>
          </Reveal>
