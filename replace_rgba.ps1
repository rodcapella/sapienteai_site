...
foreach ($file in $files) {
  ...
  foreach ($rgb in $colorMap.Keys) {
    ...
    # ── 1. Simple Tailwind utility: prefix-[rgba(R,G,B,A)]
    #    → prefix-[var(--brand-X)]/[A]
    $twRegex = '(?<prefix>(?:dark:|hover:|focus:|active:|group-hover:)?' +
               '(?:bg|text|border|ring|from|to|via|fill|stroke|outline|divide|accent|decoration|caret|placeholder|shadow-color))' +
               '-\[rgba\(' + $rgbEscaped + ',(?<alpha>[0-9.]+)\)\]'

    $content = [regex]::Replace($content, $twRegex, {
      param($m)
    ...
    # ── 2. Shadow value containing rgba: shadow-[... rgba(R,G,B,A) ...]
    #    → shadow-[... color-mix(in_srgb,var(--X)_P%,transparent) ...]
    $shadowRgbaRegex = '(?<=shadow-\[[^\]]{0,200})rgba\(' + $rgbEscaped + ',(?<alpha>[0-9.]+)\)'

    $content = [regex]::Replace($content, $shadowRgbaRegex, {
      param($m)
      $alpha = $m.Groups['alpha'].Value
      $pct   = Convert-PctStr $alpha
      "color-mix(in_srgb,$cssVar ${pct}%,transparent)"
    })
  }
